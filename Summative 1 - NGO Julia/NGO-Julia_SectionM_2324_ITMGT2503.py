...
wordcount_dictionary = dict(wordcount_dictionary)
...
def variable(f, r, c):
    ...
    optimal_R = 0
    optimal_C = 0
    constraint1 = 1000*f + 600*r + 750*c <= 18000
    constraint2 = 500*f + 750*r + 300*c <= 12000
    constraint3 = 50*f + 45*r + 75*c <= 1500
    constraint4 = f >= 0
    constraint5 = r >= 0
    constraint6 = c >= 0
    
    if constraint1 and constraint2 and constraint3 and constraint4 and constraint5 and constraint6 == True:
        optimal_F = f
        optimal_R = r
        optimal_C = c
        optimal_Z = 10*optimal_F + 12*optimal_R + 16*optimal_C
            
    return optimal_F, optimal_R, optimal_C, optimal_Z
...
def Collatz_winner(number_list):
    ...
    for i in number_list:
        #using funct from kanina 
        result = Collatz(i)
        if (result["collatz_length"] > max_length and result["max_collatz"] > max_max):
            
            max_length = result["collatz_length"]
            max_max = result["max_collatz"]
            winner = i
